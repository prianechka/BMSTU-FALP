domains    
	...
	price = integer
	name = string
	ownerSubject = building(name, price); region(name, price); transport(name, price); car(name, carColor, price).
	int_list = integer*

predicates    
	nondeterm phoneRecord(surname, phoneNumber, address)
	nondeterm owner(surname, ownerSubject)
 	nondeterm bankRecord(surname, bankName, account, sum)
 	nondeterm allPersonObj(surname, name)
 	nondeterm allPersonObjWithPrice(surname, name, price)
 	nondeterm allPersonSumObj(surname, price)
 	nondeterm sumlist(int_list, price)
 	nondeterm merge_list(int_list, int_list, int_list)
clauses 
	...
	phoneRecord("Prianishnikov", 899999, address("Krasnoyarsk", "Mate Zalki", 18, 49)).         
   	...
   	% Third ex
   	sumlist([], 0).
   	sumlist([X|Xs], Sum) :- sumlist(Xs, SumTail), Sum = X + SumTail.
   	
   	merge_list([],L,L ).
   	merge_list([H|T],L,[H|M]):- merge_list(T,L,M).
   	
   	allPersonSumObj(Surname, Sum) :- findall(Price, owner(Surname, building(_, Price)), AllPriceBuild),
   					findall(Price, owner(Surname, car(_,  _, Price)), AllPriceCar), 
   					findall(Price, owner(Surname, region(_, Price)), AllPriceRegion), 
   					findall(Price, owner(Surname, transport(_, Price)), AllPriceTransport), 
   					merge_list(AllPriceBuild, AllPriceCar, All1), 
   					merge_list(All1, AllPriceRegion, All2), 
   					merge_list(All2, AllPriceTransport, All), 
   					sumlist(All, Sum).

goal
    %allPersonObj("Prianishnikov", Name). 
    %allPersonObjWithPrice("Prianishnikov", Name, Price).
    %allPersonSumObj("Prianishnikov", Sum).
    allPersonSumObj(A, Sum).
